...
test("7. 新報名入口用項目清單，不再受單一 registrationFormType 限制", () => {
  ...
  // 正式實作：主活動群組回傳其 items，畫面以 group.items 多選勾選（見
  // NewActivityRegistrationDialog.tsx），並改走整批 /api/registrations/batch，
  // 不再受單一 registrationFormType 限制。
  assert.equal(dialog.includes("group.items"), true, "以項目清單多選");
  assert.equal(dialog.includes("/api/registrations/batch"), true, "整批建立 API");
...
test("11. 額外寶袋是獨立項目、可多筆、可自訂名稱", () => {
  ...
  // 正式實作以 canName 判斷可自訂名稱（POCKET／SPONSOR），見 NewActivityRegistrationDialog.tsx。
  assert.equal(/canName[\s\S]{0,80}POCKET/.test(dialog), true);
