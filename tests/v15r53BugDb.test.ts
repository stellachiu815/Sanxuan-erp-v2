...
async function load() {
  ...
  return { prisma, templeEvents, reg, pricing, activityReg, lantern };
}
...
dbTest("Bug1+2：草稿年度燈顯示成員現值；未收款(amountPaid=0)即可確認；應收讀 item", async () => {
  const { prisma, templeEvents, reg, pricing, activityReg, lantern } = await load();
  const year = 8901;
  ...
  try {
    ...
    // 成員有完整生日（農曆年西元1957＝民國46、7/18）與性別。
    ...
    const r = await reg.registerItemsBatch([{ memberId: m.id, registrationItemTypeId: gm, year }], "測試", null);
    ...
    // 太歲：民國46(生肖雞)於年度(視活動年度)可能犯或不犯；至少不因無快照而恆為 null——此處只驗生肖/虛歲/生日。

    // ── Bug1 應收：批次應收讀 item（500），非 LanternRegistration ──
    assert.equal(batch!.amountDue, 500, "應收＝年度燈 item 500");
    assert.equal(batch!.amountPaid, 0, "未收款");
    assert.equal(batch!.amountUnpaid, 500, "未收 500");

    // ── Bug2：未收款即可確認（validateForConfirm 通過；不依賴 amountPaid、不需 LanternRegistration）──
    const item = await prisma.ritualRegistrationItem.findFirst({ where: { registrationItemType: { key: "LANTERN_GUANGMING" }, ritualRecord: { year } } });
    ...
    const item2 = await prisma.ritualRegistrationItem.findFirst({ where: { registrationItemType: { key: "LANTERN_GUANGMING" }, ritualRecord: { year } } });
