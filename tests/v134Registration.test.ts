...
import { formatIsoDateToMinguoLong, formatLunarDateToMinguoLong } from "../src/lib/minguoDate";
...
test("17. convert API 的相容欄位 solarFormatted／lunarFormatted 走共用民國函式", () => {
  /**
   * 這一項只驗證 convert API 自己：相容欄位可保留，但值一律走共用民國函式。
   * BirthdayField 是否依賴這些欄位，是另一件事，拆到 17c 驗證，不混在一起。
   */
  const convert = readFileSync(
    join(ROOT, "src/app/api/birthday/convert/route.ts"),
    "utf-8"
  );
  // 國曆相容欄位用 formatIsoDateToMinguoLong（與詳情頁、BirthdayField 同一支）。
  assert.equal(
    convert.includes("solarFormatted: formatIsoDateToMinguoLong(toIsoDateString(solarDate))"),
    true,
    "solarFormatted 必須用 formatIsoDateToMinguoLong"
  );
  // 農曆相容欄位用 formatLunarDateToMinguoLong。
  assert.equal(convert.includes("lunarFormatted: formatLunarDateToMinguoLong({"), true);
  // 不得再殘留任何西元格式函式。
  assert.equal(/formatLunarDate\(/.test(convert), false, "不得再用西元 formatLunarDate");
  assert.equal(convert.includes("formatSolarDate"), false, "不得再用西元 formatSolarDate");
  assert.equal(convert.includes("formatMinguoDateLong"), false, "國曆改走 formatIsoDateToMinguoLong");
});

// ---- 農曆生日也用民國年（本輪補正）----

test("17b. 農曆 1972 年 7 月 7 日 → 民國61年七月初七", () => {
  assert.equal(
    formatLunarDateToMinguoLong({ year: 1972, month: 7, day: 7, isLeapMonth: false }),
    "民國61年七月初七"
  );
});
...
test("17b. 農曆閏月正確顯示「閏」", () => {
  assert.equal(
    formatLunarDateToMinguoLong({ year: 1972, month: 7, day: 7, isLeapMonth: true }),
    "民國61年閏七月初七"
  );
  // 民國元年、正月初一、廿九、三十邊界
  assert.equal(
    formatLunarDateToMinguoLong({ year: 1912, month: 1, day: 1, isLeapMonth: false }),
    "民國1年正月初一"
  );
  assert.equal(
    formatLunarDateToMinguoLong({ year: 1972, month: 12, day: 30, isLeapMonth: false }),
    "民國61年十二月三十"
  );
});

test("17b. 農曆空值 / 缺年份 → 空白（不得只顯示月日、不得西元）", () => {
  assert.equal(formatLunarDateToMinguoLong({ year: null, month: 7, day: 7 }), "");
  assert.equal(formatLunarDateToMinguoLong({ year: 1972, month: null, day: 7 }), "");
  assert.equal(formatLunarDateToMinguoLong({ year: 1972, month: 7, day: null }), "");
  assert.equal(
    formatLunarDateToMinguoLong({ year: undefined, month: undefined, day: undefined }),
    ""
  );
});

test("17b. 農曆無效資料不得顯示 Invalid Date（回空白）", () => {
  for (const bad of [
    { year: 1972, month: 13, day: 7 },
    { year: 1972, month: 0, day: 7 },
    { year: 1972, month: 7, day: 31 },
    { year: 1972, month: 7, day: 0 },
    { year: NaN, month: 7, day: 7 },
    { year: 1972, month: 1.5, day: 7 },
  ]) {
    const out = formatLunarDateToMinguoLong(bad);
    assert.equal(out, "", `${JSON.stringify(bad)} 應回空白`);
    assert.equal(out.includes("Invalid"), false);
  }
});

test("17b. 詳情頁農曆欄位改用民國共用函式，不再直接印 lunarBirthDisplay", () => {
  assert.equal(
    DEVOTEE_PAGE.includes("formatLunarDateToMinguoLong({"),
    true,
    "農曆顯示必須經過 formatLunarDateToMinguoLong"
  );
  // 舊寫法（直接印 b.lunarBirthDisplay）不得殘留在農曆欄位
  assert.equal(
    /農曆：<span[^>]*>\{b\.lunarBirthDisplay \?\? "未填寫"\}/.test(DEVOTEE_PAGE),
    false,
    "農曆欄位不得再直接輸出西元格式字串"
  );
});

test("17c. 生日編輯區即時預覽（BirthdayField）國曆與農曆都用民國共用函式", () => {
  const field = readFileSync(
    join(ROOT, "src/components/birthday/BirthdayField.tsx"),
    "utf-8"
  );
  // 國曆：formatIsoDateToMinguoLong(result.solarDate)
  assert.equal(
    field.includes("formatIsoDateToMinguoLong(result.solarDate)"),
    true,
    "預覽國曆必須用 formatIsoDateToMinguoLong"
  );
  // 農曆：formatLunarDateToMinguoLong({...result.lunar...})
  assert.equal(field.includes("formatLunarDateToMinguoLong({"), true, "預覽農曆必須用 formatLunarDateToMinguoLong");
  // 不得再用伺服器回傳的 *Formatted 字串顯示（那是西元來源）
  assert.equal(field.includes("result.solarFormatted"), false, "不得再顯示 result.solarFormatted");
  assert.equal(field.includes("result.lunarFormatted"), false, "不得再顯示 result.lunarFormatted");
  // 不得自行拼字串或殘留舊的西元函式
  assert.equal(/formatLunarDate\(/.test(field), false, "不得使用西元的 formatLunarDate");
});

test("17c. 三處（詳情頁／編輯預覽／convert API）共用同一組民國格式函式", () => {
  const convert = readFileSync(join(ROOT, "src/app/api/birthday/convert/route.ts"), "utf-8");
  const field = readFileSync(join(ROOT, "src/components/birthday/BirthdayField.tsx"), "utf-8");
  for (const src of [DEVOTEE_PAGE, field, convert]) {
    assert.equal(src.includes("formatIsoDateToMinguoLong"), true);
    assert.equal(src.includes("formatLunarDateToMinguoLong"), true);
  }
  // convert 不得再用西元 formatLunarDate / formatSolarDate / formatMinguoDateLong(Date)
  assert.equal(/formatLunarDate\(/.test(convert), false);
  assert.equal(convert.includes("formatSolarDate"), false);
});
...
test("18. limit 非法值會 reject（async 函式的 throw 是 rejected promise）", async () => {
  // mapWithConcurrency 是 async，內部 throw 會變成 rejected promise，
  // 必須用 await assert.rejects，不能用 assert.throws。
  await assert.rejects(mapWithConcurrency([1], 0, async (x) => x), /limit/);
  await assert.rejects(mapWithConcurrency([1], -1, async (x) => x), /limit/);
});
