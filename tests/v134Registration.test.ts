...
test("17. 編輯表單國曆預覽（convert API）也回民國格式，不回西元", () => {
  ...
  // 農曆預覽也必須改用民國共用函式，不得再用西元的 formatLunarDate。
  assert.equal(convert.includes("formatLunarDateToMinguoLong"), true);
  assert.equal(
    /lunarFormatted: formatLunarDate\(/.test(convert),
    false,
    "農曆預覽不得再用西元格式的 formatLunarDate"
  );
});
