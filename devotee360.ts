...
async function getDonationStats(memberId: string, now: Date) {
  ...
  // 祭改
  // 修正：這裡的 purificationEntries（見上方 prisma.purificationEntry.findMany）沒有
  // include ritualRecord，所以型別上只有純量欄位（含 ritualRecordId 這個必填外鍵），
  // 沒有 ritualRecord 這個關聯物件。PurificationEntry.ritualRecordId 在 schema 裡是必填
  // 欄位（`ritualRecordId String`，不可為空），每一筆資料一定有值，所以改用
  // p.ritualRecordId 這個一定存在的純量欄位做同一個判斷，執行結果（一定為真、一律採用
  // toRocYear(p.createdAt)）跟原本完全一樣，不需要多 include 一次關聯、不改 schema、
  // 不改業務邏輯。
  for (const p of purificationEntries) {
    if (p.feeStatus === "CHARGEABLE") {
      byCategory.祭改.push({ year: p.ritualRecordId ? toRocYear(p.createdAt) : currentYear, received: Number(p.amountPaid), due: Number(p.amountDue ?? 0) });
    }
  }
