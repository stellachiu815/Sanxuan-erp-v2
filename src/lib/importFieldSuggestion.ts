...
const DEVOTEE_PRECHECK_FIELDS: TargetFieldDef[] = [
  ...
  /**
   * ⚠️ V13.2 只調整**顯示文字**，不改 key、不改 alias、不改對應邏輯。
   *
   * 舊標籤是「普渡牌位資料筆數（數量，僅驗證）」——「普渡」二字讓使用者
   * 誤以為這個欄位與年度普渡活動報名有關，進而以為系統的普渡牌位只支援
   * 歷代祖先與乙位正魂兩種。
   *
   * 實際上這是**家戶 Excel 的核對欄位**，核對的是這一戶「固定登記」的
   * 牌位筆數，而家戶固定牌位本來就只有歷代祖先與乙位正魂兩類。
   * 冤親債主、無緣子女、寶袋屬於每年度普渡報名內容，不由家戶 Excel 匯入。
   *
   * Excel 標題列仍然是「普渡牌位資料筆數」，比對用的 alias 維持不變
   * （見下方 FIELD_ALIASES），所以正式檔案的自動對應完全不受影響。
   */
  { key: "tabletCount", label: "家戶固定牌位筆數（歷代祖先＋乙位正魂，僅驗證）" },
