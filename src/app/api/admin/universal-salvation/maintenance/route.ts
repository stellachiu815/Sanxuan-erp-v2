 ...
 *   POST body: { action: "worship-dedup" | "household-address", commit?: boolean, confirm?: boolean }
 *   commit 省略／false → 預覽（不寫入）。commit=true 需 confirm=true 才執行。
 */
...
export async function POST(request: NextRequest) {
  ...
  let body: { action?: string; commit?: boolean; confirm?: boolean };
