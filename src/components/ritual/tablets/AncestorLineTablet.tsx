...
/**
 * 歷代祖先牌位模板（V4.0 建立暫時版型，V4.1 可套版，V13.1 加入陽上人與
 * 牌位地址並全面國字化）。
 *
 ...
 *   格線決定，不要在這裡寫死尺寸）。
 * - 字體請透過 TABLET_FONT_FAMILY（見 ./shared.ts）套用。
 * - **文字一律取自 toPrintableTablet() 的結果**，不要在這裡自行轉換數字、
 *   也不要自行附加「叩薦」（V13.1 指令六、十二：轉換只能有一套）。
 */
export default function AncestorLineTablet({ entry }: { entry: PrintTabletEntry }) {
  const p = toPrintableTablet(entry);

  return (
    <div
      className="tablet-card flex h-full w-full items-center justify-center gap-2 border-4 border-double border-ink bg-white px-4 py-6"
