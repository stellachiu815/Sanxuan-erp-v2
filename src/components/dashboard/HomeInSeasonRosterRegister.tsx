...
function entryFor(activityType: string, id: string): { href: string; verb: string; tone: string; icon: string } {
  if (activityType === "ANNUAL_LANTERN") return { href: `/annual-lantern-register/${id}`, verb: "快速點燈·年度燈", tone: "bg-yolk-75", icon: "🏮" };
  if (activityType === "PALACE_LANTERN") return { href: `/roster-register/${id}`, verb: "快速報名·宮燈", tone: "bg-mist-75", icon: "🏮" };
  return { href: `/roster-register/${id}`, verb: "快速報名·補庫", tone: "bg-sage-75", icon: "🏪" };
}
      ...
      {open.map((e) => {
        ...
        return (
          <Link
            key={e.id}
            href={href}
            className={`flex min-h-[88px] flex-col items-center justify-center gap-2 rounded-3xl ${tone} p-4 text-center shadow-card transition hover:shadow-pop`}
          >
            <span className="text-2xl" aria-hidden>{icon}</span>
            <span className="text-sm font-medium text-ink">{verb}</span>
          </Link>
        );
      })}
    </>
