/**
 * 蘇玉蘭重複乙位正魂：重新封存指定的那一筆（dry-run 預設；--commit 才執行）。
 *
 *   保留（不碰）：cmse81x4t0038fd1s8kaicj0n（F00221｜吳念騏｜蘇玉蘭乙位正魂）
 *   重新封存    ：cmsdjyb1l00fofv1simt5f0fz（F00211｜吳家｜蘇玉蘭乙位正魂）
 *
 * 走既有正式流程 deleteUniversalSalvationEntry()：連動取消**未收款** RRI、軟刪其預設 TABLET／
 * 基本 POCKET（V34.3B 連動）。不硬刪、不碰保留那一筆、不碰額外寶袋（isExtra=true，如「江士耀」）、不動財務。
 *
 * 安全前置檢查（任一不符即中止、絕不寫入）：
 *   - 目標 entry 存在、未封存、category=INDIVIDUAL_SOUL、名稱含「蘇玉蘭」、屬 115 普渡。
 *   - 其關聯 RRI amountPaid = 0；預設 TABLET／POCKET printCount = 0。
 *   - 無收據、無收款分配（payment_allocations／receipt_lines 參照其 RRI）。
 *   - 保留那一筆存在且不會被觸及。
 *
 *   npx tsx scripts/reArchiveSuYulanDuplicate.ts            # Dry-Run
 *   npx tsx scripts/reArchiveSuYulanDuplicate.ts --commit   # 正式封存
 */
import { prisma } from "../src/lib/prisma";
import { deleteUniversalSalvationEntry } from "../src/lib/ritual";

const KEEP_ID = "cmse81x4t0038fd1s8kaicj0n";
const DELETE_ID = "cmsdjyb1l00fofv1simt5f0fz";
const NAME_HINT = "蘇玉蘭";
const YEAR = 115;
const OPERATOR = "系統：蘇玉蘭重複牌位重新封存";
async function q<T>(sql: string, ...p: unknown[]): Promise<T[]> { return prisma.$queryRawUnsafe<T[]>(sql, ...p); }

async function main() {
  const commit = process.argv.includes("--commit");
  console.log(`=== 蘇玉蘭重複牌位重新封存（${commit ? "COMMIT" : "DRY-RUN"}）===`);
  console.log(`保留：${KEEP_ID}\n封存：${DELETE_ID}\n`);

  const info = async (id: string) => (await q<{ eid: string; disp: string; cat: string; del: Date | null; hh: string; hhname: string | null; year: number; act: string }>(
    `SELECT e."id" AS eid, e."displayName" AS disp, e."category"::text AS cat, e."deletedAt" AS del,
            rr."householdId" AS hh, h."name" AS hhname, rr."year" AS year, rr."activityType"::text AS act
     ...
     LEFT JOIN "households" h ON h."id"=rr."householdId" WHERE e."id"=$1`, id))[0];
  ...
  console.log(`要封存：${del.disp}｜category=${del.cat}｜家戶=${del.hh}（${del.hhname ?? "?"}）｜${del.act}/${del.year}｜deletedAt=${del.del ? "已封存" : "未封存"}`);
  console.log(`要保留：${keep.disp}｜category=${keep.cat}｜家戶=${keep.hh}（${keep.hhname ?? "?"}）｜deletedAt=${keep.del ? "已封存" : "未封存"}`);

  if (del.del) abort("目標 entry 已是封存狀態，無需再封存");
  if (del.cat !== "INDIVIDUAL_SOUL") abort(`目標 entry 類別=${del.cat}，非乙位正魂`);
  if (!del.disp.includes(NAME_HINT)) abort(`目標 entry 名稱「${del.disp}」不含「${NAME_HINT}」`);
  ...
  // 預設 TABLET／基本 POCKET printCount；額外寶袋（isExtra=true）不受連動。
  const objs = await q<{ id: string; itemType: string; isExtra: boolean; printCount: number; printName: string | null; del: Date | null }>(
    `SELECT "id","itemType"::text AS "itemType","isExtra","printCount","printName","deletedAt" AS del FROM "additional_print_items" WHERE "sourceEntryId"=$1`, DELETE_ID);
  const defaults = objs.filter((o) => !o.isExtra && !o.del);
  const extras = objs.filter((o) => o.isExtra && !o.del);
  ...
  console.log(`額外寶袋（isExtra=true，不受此封存連動）：${extras.map((o) => o.printName ?? "?").join("、") || "無"}`);
  console.log(`收據行 receipt_lines=${rl}｜收款分配 payment_allocations=${alloc}`);

  if (paidSum !== 0) abort(`amountPaid=${paidSum}≠0，禁止封存`);
  if (maxPrint !== 0) abort(`printCount=${maxPrint}≠0，禁止封存`);
  if (alloc + rl > 0) abort(`偵測到收據/收款分配（收據行 ${rl}／分配 ${alloc}），禁止封存`);
  console.log(`✓ amountPaid=0、printCount=0、無收據、無收款分配、未封存 → 可安全封存。`);

  console.log(`\n將執行 deleteUniversalSalvationEntry("${del.hh}", ${YEAR}, "${DELETE_ID}")：`);
  console.log(`  - 軟刪 entry ${DELETE_ID}`);
  console.log(`  - 連動取消其未收款 RRI（已收款則保留）`);
  console.log(`  - 連動軟刪其預設 TABLET／基本 POCKET`);
  console.log(`  - 不觸及保留的 ${KEEP_ID}、不碰額外寶袋、不硬刪、不動財務`);
